...
9. **実装を渡す前にツリーをクリーンにする。** 各タスクの指示書は「クリーンなツリーから開始」を着手条件にしている（ガードレール5）。したがって**指示書そのものや記録の更新を先にコミットしてから実装を委譲する。** 2026-08-14 にこの順序を2回誤り、haiku が task_003 と task_004a の着手時点で正しく停止した（原因はどちらも Opus が直前に書いた `docs/` の変更を未コミットのまま実装を依頼したこと）。停止自体は規律が働いた証拠だが、往復が無駄になるので委譲の直前に `git status --porcelain` が空であることを確認する。
