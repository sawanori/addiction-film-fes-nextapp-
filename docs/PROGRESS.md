...
## 13. 初めての本番コンテンツ編集と baseline の扱い（2026-08-19）

利用者の依頼で、トップページのナビゲーター紹介の詳細段落（`<p class="navigator__d">`）を
**よしひろまさみち氏本人のコメント**に差し替えた。移植完了後、初めての意図的な文言変更。

### やったこと

1. `content/index.json` の `navigator.d` を更新（同梱データ＝DB障害時のフォールバック兼 seed の元）
2. **管理APIを通して** dev と本番の両方の DB を更新（履歴が残る形にするため。直接SQLは使わない）
   - dev: 版5 → 版6、本番: 版1 → 版2、メモ「ナビゲーターのコメントを掲載」
3. `npm run verify:text` で差分を確認 → **意図した1件だけ**（`8.1.0.1.6` の `<p>` のテキスト）
4. `npm run verify:text -- --update` で baseline を更新

### baseline を更新する判断について

移植の不変条件（着手前と完全一致）は**移植が終わった時点の証明**であり、
公開後に管理画面から内容を直せば当然ずれる。ずれたまま放置すると回帰検証が常に失敗し、
「意図しない変更」を検出する道具として使えなくなる。そこで次の運用にする。

- **意図的な文言変更を入れたら、その差分が意図どおりの1件だけであることを確認してから baseline を更新する**
- 更新は同じコミットに含め、コミットメッセージに「何を変えたか」を書く
- 移植時点の完全一致の証拠は git 履歴に残る（`verification/baseline.fingerprint.json` の変遷）

なお baseline の HTML スナップショット8件も同時に変わったが、中身はフレームワークが吐く
スクリプト名・RSC ペイロード・属性の並び順で、可視テキストではない
（指紋比較はハッシュを伏せ属性を集合で見るため、他7ページは完全一致のままだった）。

### 併せて起きたこと: `node_modules` が空になっていた

この作業中に `node_modules` が0件になっているのを発見した（ディスク空きが 7.7Gi → 16Gi へ増えており、
何かが削除したとみられる）。`npm ci` で復旧し、`next` 16.3.0 / `@libsql/client` 0.17.4 を確認した。
**本番は影響を受けていない**（デプロイ済みの Worker は自己完結しているため）。

### 未対応（利用者の判断待ち）

掲載した文に **「させてくれるる」** という表記がある（「くれる」の誤記の可能性）。
本人のコメントとして預かった文章のため、**こちらでは直さずそのまま掲載した**。
直す場合は管理画面から1文字消すだけで済む。


---

## 14. パスワードの変更（2026-08-19）

利用者の指示で管理画面のパスワードを変更した。**平文はリポジトリのどこにも書かない。**

### やったこと

1. `scripts/hash-password.mjs` で **dev 用と本番用に別々のソルト**のハッシュを生成した
   （反復回数100,000。標準入力で渡し、コマンドライン引数にもファイルにも平文を残さない）
2. `.dev.vars` と `.dev.vars.prod-reference` の `ADMIN_PASSWORD_PBKDF2` を差し替えた（どちらも600）
3. 本番は `npx wrangler secret put ADMIN_PASSWORD_PBKDF2` で差し替えた
4. **`admin_settings.session_version` を +1 した**（dev / 本番とも 1 → 2）。
   計画 §9.1 の「パスワード変更時は全端末を失効させる」に従い、変更前に発行済みの
   セッションCookieをすべて無効にするため

### 検証（Opus が実測）

| 確認 | ローカル | 本番 |
|---|---|---|
| 新しいパスワードでログイン | 200 `{"ok":true}` | 200 `{"ok":true}` |
| **古いパスワードでログイン** | **401 `{"error":"invalid"}`** | **401 `{"error":"invalid"}`** |
| 新しいCookieで `/admin` | 200 | 200（編集画面も200） |
| **変更前に発行した署名Cookie（`ver`=1）** | — | **401**（session_version の +1 が効いている） |

コードは変更していないため再デプロイは不要（シークレットは即時反映される）。
...
### 15.5 予告編以外の欠落入力欄も同日に開いた（35件）

当初「約100件」と書いたが、**分類して数え直すと35件**だった（残り85件は多態ブロックで、
出さないのが正しい）。26パスを `ALWAYS_SHOWN` に足して全部開いた。内訳は
表示アニメの遅れ17・見た目の種類7・programme 用クレジット5・フッターのリンク先4・
リンク用のID1・本文2の見た目1。

**足すだけでは済まない項目が2つあった。** どちらも空欄で保存されたときに公開DOMが
変わってしまうもので、先に描画側を直した。

- `components/Films.tsx` の `metaProgramme`: **空配列は真**なので、書きかけて消すと
  programme のクレジットが丸ごと消えるところだった。`?.length` で見るよう変更
- `components/SiteFooter.tsx` の `hasLink`: `"base" in item` という**キーの有無**だけの
  判定で、空文字の `base` が `href=""` のリンクになる。中身があるかで判定するよう変更。
  既存リンクのリンク先を空にされた場合にも効く（元からあった穴）

`delay` / `variant` / `id` / `p2Cls` は描画側が `x ? … : {}` か等値比較のため手当て不要だった。

検証は「新しく開けた35欄をすべて空欄のまま保存した状態」を dev DB に直接作り、公開8ルートを
baseline と突き合わせて完全一致を確認した（空欄＝未設定が DOM 上で等価であることの証明）。
そのうえで、フッターのテキスト項目「よみうりホール」を管理画面からリンク化できること、
04 の programme 用クレジットを空にすると短縮版に戻ることも実測した。

`privacy` / `terms` の多態ブロック85件は**開けない**（現在の挙動が正しい）。
