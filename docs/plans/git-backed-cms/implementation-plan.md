...
- **A5**: Vercel での1回のビルド〜反映は 1〜2分。**確定した**（task_012 の実測: 保存リクエストから
  公開反映まで 31秒〜102秒。ビルド 19〜22秒、差はキュー待ち。`docs/deploy-vercel.md` §9）。
...
「1〜2分」は task_012 の実測（31秒〜102秒）で裏付けた。文言はそのまま。
...
7. **履歴表のメモ欄が長くなる。** 開発者が git で入れたコミットは本文（コミットメッセージの
   2行目以降）がそのまま「メモ」に出るため、数百文字の行になる（task_012 で実測。管理画面からの
   保存は短い）。表示を数行で切り詰めるか、`content(<key>):` で始まらないコミットは本文を出さない、
   のどちらかを別案件で決める。**（2026-09-03 に対応済み: 履歴表を `<details>` で折りたたみ、
   `content(<key>):` で始まらないコミットは本文を出さない）**
8. **Vercel プロジェクトを CLI / API で作ると Framework Preset が未設定になり全ページ 404 になる**
   （task_012 で実測。`framework: nextjs` を PATCH して再デプロイで直った）。先方はダッシュボードの
   Import で作るので自動で入るが、`docs/deploy-vercel.md` §1 に注意書きを残した。
9. **検証用プロジェクトの扱い。** task_012 で作る `addiction-film-fes-staging` を常設のステージングに
